...
# This script derives bootstrap CIs for the scores and generates boxplots. This should be the version used in the paper!
...
for(yy in years)
{
  ...
  for(ee in 'K')
  {
    ...
    sc_p = ggplot(data = dt_boot[est == ee & year == yy],
                   aes(x = model, y = mean_score)) + geom_boxplot(show.legend = FALSE)
