...
### Produce results for the F-estimatore simulation study ###
...
# for running or skipping parts of the scripts:

get_beta = FALSE # derive the betas for the Strauss process that lead to expected number of 50 points.
generate_ppp = FALSE # generate simulation examples of 10.000 point patterns from each of the models.
...
N = 2000 # only use the first N point patterns (per model) in ppp_list
...
{
  ret_vec = x
  ret_vec[is.na(ret_vec)] = 1
  return(ret_vec)
}

dt = dt[,lapply(.SD,FUN = rem_nas )]
...
crps_first_term_new = function(yinds,xinds,R_inds = 1:K,models = mods)
{
  whichmods = match(models,mods)
  ret_dt = data.table()
  for(om in whichmods)
  {
    ...
    {
      xinds_temp = xinds + N*(pm-1)
      temp_yx = apply(abs_errs[yinds_temp,xinds_temp,],2,FUN = mean)
      temp_dt = rbindlist(list(temp_dt,
                               data.table(crps1 = temp_yx,
                                          pred_mod = c("Str1", "Str2",  "hP",    "LGCP1", "LGCP2")[pm],
                                          R=weight_cutoffs)))
    }
    temp_dt[,obs_mod := c("Str1", "Str2",  "hP",    "LGCP1", "LGCP2")[om]]
...
for(nx in nxs)
{
  ...
  for(rrr in 1:rr)
  {
    ...
    yinds = sample(N,ny,replace = TRUE)
    temp1 = crps_first_term_new(yinds,xinds)
    temp1[,r:=rrr][,nx := nx][,ny := ny]
    dt_1 = rbindlist(list(dt_1,temp1))
  }
}
